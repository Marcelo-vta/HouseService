...
public class ClickPlayerStarter : MonoBehaviour
{
    ...
    void TrySelectPlayerByClick()
    {
        ...
        if (clicked.CompareTag(playerTag))
        {
            ...
            {
                // fallback: usa a ordem/hierarchy ou name se n�o houver PlayerIdentity
                Debug.LogWarning("Player clicado sem PlayerIdentity. Usando fallback pelo nome.");
                pname = clicked.name;
            }

            // salva a sele��o no GameController (para ser lida na pr�xima cena)
            GameController.SetSelectedPlayer(pid, pname);

            // chama a fun��o IniciaJogo no seu MenuActions (procura componente)
            var menu = FindFirstObjectByType<MenuActions>();
            if (menu != null)
            {
                // se o seu IniciaJogo original n�o aceita par�metros, apenas chama
                menu.IniciaJogo();

                // Se quiser, voc� pode criar uma sobrecarga em MenuActions que aceite (int id) e chamar aqui:
                // menu.IniciaJogo(pid);
            }
            else
            {
                // fallback: se voc� n�o tiver MenuActions, chama diretamente o GameController e o SceneController
                Debug.LogWarning("MenuActions n�o encontrado. Chamando GameController.Init() e carregando cena diretamente.");
                GameController.Init();
        ...
        {
            // clique em outro objeto (n�o player) -> opcional: feedback
